...
const dataLog1 = async ({ IDUSER }) => {
  ...
  try {
    ...
    const res = await axios.get(`https://backend-tes-taupe.vercel.app/settanam/getbyuser/${IDUSER}`)
...
const getAllDataByID = async (id) => {
  try {
    const res = await axios.get(`https://backend-tes-taupe.vercel.app/alat/get/${id}`)
    const data = res.data.data
